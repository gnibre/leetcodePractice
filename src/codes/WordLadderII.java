...
/**
 *
 * word dot, can have two/more farther like  hot, dog.
 * 
 * if hot and dog are both at level2 of the searching tree,  we shall save both path from start word to the word "dot".
 * 
 * if we mark word "dot" as "visited" and discard it, we lose one or more case, they are all shortest transformations.
 * 
 * however, if say "dot" have two farther, one "hot" at level 2, and "dog" at level 3, we can discard "dog" case, as it can't be
 * shortest one.
 * 
 * so we shall record two things in the search tree :  level, parent.
 */
public class WordLadderII {
    
    // record first time appear level when reached.
    private HashMap<String,Integer> wordFirstAppearLevelMap = new HashMap<String,Integer>();
    private HashSet<String> endReverseSet = new HashSet<String>();
    int curId = 1;
...
//    	ArrayList<ArrayList<String>> res = findLadders("nanny","aloud",dict);
    ...
    /**
     * still use bfs, search level by level, record parent of each node. for the final result output.
     * 
     * when a word is found, we check it's level,  if it has already exit at higher level, don't add to the tree.
     * 
     * when the target word is found at this level, we out put all hte result found in this level and work's done.
     * 
     */
    public ArrayList<ArrayList<String>> findLadders(String start, String end, HashSet<String> dict) {
        
        if(start==null||end==null||start.length()!=end.length()){       // String.length()!!!!!  is a function.
            return null;
        }
        
        // shall we add start and end to the dict?
//        start = start.toLowerCase(); // function   toLowerCase()!!!!  
//        end = end.toLowerCase();
        ...
        wordFirstAppearLevelMap.put(start,0);
//        ArrayList<TreeNode> nodeList = new ArrayList<TreeNode>();
        LinkedList<TreeNode> nodeList = new LinkedList<TreeNode>();
        nodeList.add(root);
        while(foundTargetLevel<0&&nodeList.size()>0){
            currentLevel++; //start search from level 1.
//            System.out.println(" cl: "+currentLevel);
            int size = nodeList.size();
            for(int li=0;li<size;++li){
                // search all the node of next level. add to the arraylist.
                	...
                	for(n='a';n<='z';++n) {
                		if(n!=saved){
                			...
                			if(dict.contains(s)){
                				...
                				if(s.equals(end)){
                                    foundTargetLevel = currentLevel;
                                    //add this poped one back.
                                    nodeList.add(node);
                                    break;
                                    
                                    
                                }
                                if(foundTargetLevel<0){
                                    //not found, add to tree and continue searching.
                                    if(checkNodeAppear(s,currentLevel)){
                                        // can add to the pool
                                        TreeNode toAdd = new TreeNode(s,node);
                                        nodeList.add(toAdd); // we add to the same list, 
                                                            //after remove nodes of prev level, we got nodes of this level. one list is enough
                                        wordFirstAppearLevelMap.put(s,currentLevel);
                                    }else{
                                        // don't add, so do nothing.
                                    }
                                }
                ...
                // if end up with nodelist is empty and no result found,
                // : no result.
            }
        }
        ...
        for(TreeNode nd:nodeList){
        	if(endReverseSet.contains(nd.word)){
        		//hit.
        		ArrayList<String> onePath = new ArrayList<String>();
                onePath.add(end);
                while(nd!=null){
                	onePath.add(0,nd.word); // add to front. node ord is from bot to root of the tree.
                    nd = nd.parent;
                }
                result.add(onePath);
        	}
        }
        wordFirstAppearLevelMap.clear();
        nodeList.clear();
        return result;
    }
    
    
    private ArrayList<String> getResultOneTransformation(TreeNode node,String end){
//    	System.out.println(" got , last word : "+node.word+"  ");
        ArrayList<String> ret = new ArrayList<String>();
        ret.add(end);
        while(node!=null){
            ret.add(0,node.word); // add to front. node ord is from bot to root of the tree.
            node = node.parent;
        }
        return ret;
    }
    ...
    // 
    private ArrayList<String> findNeighbour(String input,HashSet<String> dict){
    ...
    /**
     * if curLevel is > the first time this node appear, return false;
     */
    private boolean checkNodeAppear(String s,int curLevel){
        if(wordFirstAppearLevelMap.containsKey(s)){
            if(wordFirstAppearLevelMap.get(s)<curLevel){
                return false;
            }
        }
        return true;
